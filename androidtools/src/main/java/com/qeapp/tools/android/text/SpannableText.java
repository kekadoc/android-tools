 ...
 * @see Annotation
 * @see com.qeapp.tools.android.R.string.string_spannable_test Example
 *
 */
public class SpannableText implements Spannable {
    ...
    private static @NonNull Spanned pullOutText(@NonNull Context context, @StringRes int id) {
        CharSequence txt = Objects.requireNonNull(QeAndroid.getText(context, id), "Error inflating resource");
    ...
    protected void applySpan_size(@NonNull Spannable text, @NonNull Region region) {
        ...
        if (!QeChars.isNumber(val.charAt(0))) throw new Wtf();
        int s = 0;
        boolean dp = false;
        for (int i = 0; i < val.length(); i++) {
            if (!QeChars.isNumber(val.charAt(i))) {
    ...
    private void findMarker(@NonNull CharSequence sequence, boolean open, int start, MarkerFinder markerFinder) {
        ...
        for (int i = start; i < sequence.length(); i++) {
            ...
            if (c == MARK_MARKER) {
                ...
                int code = QeChars.parseNumber(sequence, i + POSITION_CODE_MARK);
    ...
    private Region onFindStringFormatAnnotation(@NonNull Spanned sequence, @NonNull Annotation annotation) {
        ...
        if (QeChars.isNumber(format.charAt(1))) {
            int number = Integer.parseInt(Character.toString(format.charAt(1)));
            if (QeChars.isNumber(format.charAt(2))) throw new Wtf("Число должен быть меньще 10!");
    ...
    public String toString() {
        if (getSpannable() == null) {
            if (getSpanned() == null) return QeObjects.toSimpleString(this) + " Spanned is null";
    ...
    protected static class Region {
        ...
        @Override
        public String toString() {
            return QeObjects.toSimpleString(this) + " [ " + start  + " ... " + end  + "] key: {" + key + "} value: {" + value + "} ";
        }
    }
